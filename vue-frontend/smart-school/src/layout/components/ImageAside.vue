<template>
    <el-aside width="220px" class="image-aside" v-loading="loading">
        <div class="top">
            ...
            <AsideList :active="activeId == item.id" v-for="(item,index) in list" :key="index" @edit="handleEdit(item)" @delete="handleDelete(item.name)" @click="handleChangeActiveId(item.name)">
                {{ item.name }}
            </AsideList>
...
function getData(p = null){
    ...
    .then(res=>{
        ...
        if(item){
            handleChangeActiveId(item.name)
        }
    })
    .finally(()=>{
        loading.value = false
